...
class Robot:
    ...
    def update_state(self, action, current_velocity):
        ...
        # Remove water resistance calculation
    ...
    def get_global_states(self, robots, obstacles):
        """
        返回全局坐标系下的机器人和障碍物的状态矩阵

        Parameters:
        - robots: 所有机器人对象的列表
        - obstacles: 所有障碍物对象的列表

        Returns:
        - robots_matrix: 一个 Nx6 的矩阵，每行表示一个机器人 [x, y, vx, vy, goal_x, goal_y]
        - obstacles_matrix: 一个 Mx3 的矩阵，每行表示一个障碍物 [x, y, r]
        """
        # 初始化机器人矩阵，每行是 [x, y, vx, vy, goal_x, goal_y]
        robots_matrix = np.zeros((len(robots), 6))
        for i, robot in enumerate(robots):
            robots_matrix[i, 0] = robot.x  # 机器人全局 x 坐标
            robots_matrix[i, 1] = robot.y  # 机器人全局 y 坐标
            robots_matrix[i, 2] = robot.velocity[0]  # 机器人全局 x 方向速度
            robots_matrix[i, 3] = robot.velocity[1]  # 机器人全局 y 方向速度
            robots_matrix[i, 4] = robot.goal[0]  # 机器人目标点 x 坐标
            robots_matrix[i, 5] = robot.goal[1]  # 机器人目标点 y 坐标

        # 初始化障碍物矩阵，每行是 [x, y, r]
        obstacles_matrix = np.zeros((len(obstacles), 3))
        for j, obs in enumerate(obstacles):
            obstacles_matrix[j, 0] = obs.x  # 障碍物中心 x 坐标
            obstacles_matrix[j, 1] = obs.y  # 障碍物中心 y 坐标
            obstacles_matrix[j, 2] = obs.r  # 障碍物半径
